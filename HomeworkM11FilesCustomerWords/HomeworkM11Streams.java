...
public class HomeworkM11Streams {
	...
	public static void main(String[] args) throws Exception {
		...
		// Q13: How many customers in CA?
		System.out.print("Q13: How many customers with a name that starts with z?\nExpected: 0\n  Actual: ");
		long extraCreditCustomers = q13();
		System.out.println(extraCreditCustomers);
	...
	public static long q1() {
		// Q1: How many customers in CA?
		return customerList.stream().filter(c -> c.getState().equalsIgnoreCase("ca")).count();

	}
	public static List<Customer> q2 () {
		// Q2: Create a list of all priority customers in MA.
		return customerList.stream().filter(c -> c.getState().equalsIgnoreCase("ma") && c.isPriority()).toList();
	}
	public static double q3() {
		// Q3: How much money have all customers spent (combined)?
		return customerList.stream().mapToDouble(Customer::getAmountSpent).sum(); // placeholder: replace with your own code
	}
	public static double q4() {
		// Q4: How much money have all priority customers spent (combined)?
		return customerList.stream().filter(Customer::isPriority).mapToDouble(Customer::getAmountSpent).sum(); // placeholder: replace with your own code
	}
	public static Map<String, Customer> q5() {
		// Q5: Create a map of all WY priority customers (key=id, value=customer)
		return customerList.stream().filter(c -> c.getState().equalsIgnoreCase("wy") && c.isPriority()).collect(Collectors.toMap(Customer::getId, c -> c)); // placeholder: replace with your own code
	}
	public static double q6() {
		// Q6: What is the greatest amount of money spent by a NY priority customer?
		return customerList.stream().filter(c -> c.isPriority() && c.getState().equalsIgnoreCase("ny")).mapToDouble(Customer::getAmountSpent).max().orElse(0.0); // placeholder: replace with your own code
	}
	public static String q7() {
		//Q7: Find all customers that spent > 9000.
		// Create a comma-separated String of all customer IDs for customers that spent > 9000:
		return customerList.stream().filter(c -> c.getAmountSpent() > 9000).map(c -> String.valueOf(c.getId())).collect(Collectors.joining(",")); // placeholder: replace with your own code
	}
	public static void q8() {
		//Q8: Find any customer that has spent > 9800. 
		// Print the amount spent by the customer. If there is none, nothing should be printed.
		// Note: you can test your code with a lower amount, too.
		customerList.stream().map(Customer::getAmountSpent).filter(amountSpent -> amountSpent > 9800).forEach(spent -> System.out.print(spent + " "));
	}
	public static int q9(String[] numWords) {
		// Q9: Find the sum of the numbers represented in an String array.
		return Stream.of(numWords).mapToInt(Integer::parseInt).sum(); // placeholder: replace with your own code
	}
	public static String q10(Integer[] nums) {
		// Q10: Create a String of the numbers represented in the array, separated by semicolons.
		return Stream.of(nums).map(String::valueOf).collect(Collectors.joining(";")); // placeholder: replace with your own code
	}
	public static void q11() {
		...
		Stream.generate(() -> random.nextInt(100) + 1).filter(num -> num % 3 == 0).limit(5).forEach(num -> System.out.print(num + " "));
	...
	public static void q12(List<String> scrabbleWords) {
		...
		// Note: a method is provided below to convert from char to score.
		// Hint: you might consider writing another method to find the score of a word!
		// Create a map of words and their scores
		Map<String, Integer> wordScoresMap = scrabbleWords.stream()
				.collect(Collectors.toMap(
						word -> word,
						word -> word.chars().map(c -> charToScore((char) c)).sum(),
						(oldValue, newValue) -> oldValue,
						LinkedHashMap::new
				));

		wordScoresMap.entrySet().stream()
				.sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))
				.limit(9)
				.forEach(entry -> System.out.println(entry.getKey() + " worth " + entry.getValue() + " points"));
	}
	...
	public static long q13() {
		// Q13: How many customers with a name that starts with z?
		return customerList.stream().filter(c -> Character.toLowerCase(c.getFirstName().charAt(0)) == 'z').count();
