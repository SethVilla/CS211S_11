...
public class ProductBigDataQuestions {
    ...
    public static int q1() {
        ...
        return productList.stream().mapToInt(product -> product.getChemicals().size())
                .sum();
    }
    public static int q2() {
        // QUESTION 2: How many different companies are in the dataset?
    	 return (int) productList.stream().map(Product::getCompany).distinct().count(); // placeholder so the code will compile; replace with your own code
    }
    public static long q3() {
        // QUESTION 3: How many products have 4 or more chemicals?
    	 return productList.stream().filter(p -> p.getNumberOfChemicals() >= 4).count(); // placeholder so the code will compile; replace with your own code
    }
    public static Map<String, List<Product>> q4() {
        ...
        // Hint: use Collectors.groupingBy!
    	 return productList.stream().collect(Collectors.groupingBy(Product::getCompany)); // placeholder so the code will compile; replace with your own code
    }
    public static String q5(Map<String, List<Product>> companyProductMap) {
    	...
    	// based on size of their lists.
    	 return companyProductMap.entrySet().stream().max(Comparator.comparingInt(entry -> entry.getValue().size())).map(Map.Entry::getKey).orElse(null); // placeholder so the code will compile; replace with your own code
    }
    public static void q6( Map<String, List<Product>> chemicalProductMap) {
        ...
        chemicalProductMap.forEach((chemical, products) ->
                productList.stream()
    ...
    public static String q7() {
    	...
    	// to compare chemical names based on the size of the list of products.
    	 return chemicalProductMap.entrySet().stream()
                 ...
                 .orElse(null); // placeholder so the code will compile; replace with your own code
    }
